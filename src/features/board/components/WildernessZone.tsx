...
interface WildernessZoneProps extends React.HTMLAttributes<HTMLDivElement> {
  cards: PreyCardType[]
}
 ...
 */
export function WildernessZone({ cards, className, ...props }: WildernessZoneProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="flex flex-row justify-center gap-4 flex-wrap">
        {cards.map((card, idx) => (
          <PreyCard key={card.id || idx} card={card} />
        ))}
        {/* Fill empty slots with placeholders if needed, assuming exactly 4 for now */}
        {Array.from({ length: Math.max(0, 4 - cards.length) }).map((_, idx) => (
          <div
            key={`empty-${idx}`}
